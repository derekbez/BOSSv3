"""Meal Idea — random meal suggestion with pagination.

Yellow/Blue = prev/next page, Green = new meal.
"""
...
def _fetch(timeout: float) -> tuple[str, list[str]]:
    ...
    if not meals:
        raise RuntimeError("No meal returned")
    meal = meals[0]

    name = str(meal.get("strMeal", "Meal Idea"))
    category = str(meal.get("strCategory", ""))
    area = str(meal.get("strArea", ""))
    instructions = str(meal.get("strInstructions", "")).strip()

    parts: list[str] = []
    if category:
        parts.append(f"Category: {category}")
    if area:
        parts.append(f"Origin: {area}")

    ingredients: list[str] = []
    for i in range(1, 21):
        ing = str(meal.get(f"strIngredient{i}", "")).strip()
        meas = str(meal.get(f"strMeasure{i}", "")).strip()
        if ing:
